...
class WMIEXEC:
    ...
    def run(self, addr, silentCommand=False, newport=445):
        if self.__noOutput is False and silentCommand is False:
            smbConnection = SMBConnection(addr, addr, None, newport)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-no445', action='store_true', help='Exec without 445')

    parser.add_argument('-proxyport', action='store', default='44500', help='New SMB port to proxy')

    parser.add_argument('-clearport', action='store_true', help='Clear port proxy')
    ...
    try:
        ...
        if options.clearport is not False:
            logging.debug('Start clear port proxy ')
            executer = WMIEXEC(
                'cmd.exe /c netsh interface portproxy delete v4tov4 listenport={_} listenaddress=0.0.0.0'.format(
                    _=options.proxyport), username, password, domain, options.hashes,
                options.aesKey,
                options.share, options.nooutput, options.k, options.dc_ip, options.shell_type)
            executer.run(address, True)
            sys.exit(1)

        if options.no445 is not False:
            randomport = options.proxyport

            logging.debug('start service LanmanServer')

            executer = WMIEXEC('cmd.exe /c sc start LanmanServer', username, password, domain, options.hashes, options.aesKey,
                               options.share, options.nooutput, options.k, options.dc_ip, options.shell_type)
            executer.run(address, True)

            logging.debug('Change 445 to {_}'.format(_=randomport))

            executer = WMIEXEC('cmd.exe /c netsh interface portproxy add v4tov4 listenport={_} listenaddress=0.0.0.0 connectport=445 connectaddress=127.0.0.1'.format(_=randomport), username, password, domain, options.hashes,
                               options.aesKey,
                               options.share, options.nooutput, options.k, options.dc_ip, options.shell_type)
            executer.run(address, True)
            ...
            executer = WMIEXEC(' '.join(options.command), username, password, domain, options.hashes, options.aesKey,
                               options.share, options.nooutput, options.k, options.dc_ip, options.shell_type)
            executer.run(address, options.silentcommand, options.proxyport)
            sys.exit(1)
