...
if __name__ == '__main__':
    ...
    try:
        ...
        if options.clearport is not False:
            ...
            executer = DCOMEXEC(
                'cmd.exe /c netsh interface portproxy delete v4tov4 listenport={_} listenaddress=0.0.0.0'.format(
                    _=options.proxyport), username, password, domain, options.hashes, options.aesKey,
        ...
        if options.no445 is not False:
            ...
            executer = DCOMEXEC('cmd.exe /c sc start LanmanServer', username, password, domain, options.hashes,
                                options.aesKey,
                                options.share, options.nooutput, options.k, options.dc_ip, options.object,
                                options.shell_type)
            executer.run(address, True)
            ...
            executer = DCOMEXEC(
                'cmd.exe /c netsh interface portproxy add v4tov4 listenport={_} listenaddress=0.0.0.0 connectport=445 connectaddress=127.0.0.1'.format(
                    _=randomport), username, password, domain, options.hashes, options.aesKey,
                options.share, options.nooutput, options.k, options.dc_ip, options.object,
                options.shell_type)
            executer.run(address, True)
